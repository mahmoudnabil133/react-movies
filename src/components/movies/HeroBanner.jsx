...
export default function HeroBanner({ movie, onTrailerClick }) {
  ...
  return (
    <section className="relative w-full min-h-[70vh] flex items-end overflow-hidden -mt-[72px] pt-[72px]">
      ...
      />
      <div className="absolute inset-0 backdrop-blur-sm bg-[#0b1220]/70" />
      <div
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-16 w-full flex flex-col md:flex-row gap-8 items-end">
        <Link to={`/movie/${movie.id}`} className="shrink-0 group">
          <img
            ...
            alt={movie.title}
            className="w-44 md:w-52 rounded-xl shadow-2xl shadow-black/50 border border-white/10 transition-transform duration-300 group-hover:scale-105"
          />
        </Link>
        ...
        <div className="flex-1 space-y-4 animate-fade-in">
          <h1 className="text-3xl md:text-5xl font-bold text-white leading-tight">
            {movie.title}
            {year && <span className="text-gray-400 font-normal ms-3">({year})</span>}
          </h1>
          ...
          <p className="text-gray-300 max-w-2xl line-clamp-3 leading-relaxed">
            {movie.overview}
          </p>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium transition-all hover:shadow-lg hover:shadow-blue-600/30"
            >
              ▶ {t("watchTrailer")}
            </button>
            ...
            <Link
              to={`/movie/${movie.id}`}
              className="px-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 text-white font-medium transition-all border border-white/10"
            >
              {t("overview")}
            </Link>
          </div>
